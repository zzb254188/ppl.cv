                ...
                for (size_t y = 0; y < blockHeight; ++y)
                {
                    ...
                    {                        
                        if(cn == 1)
                            ...
                            float32x4_t res = {src[map_row[x]], src[map_row[x] + 1], 
                                               src[map_row[x] + 2], cn == 4 ? src[map_row[x] + 3] : dst_row[x*cn + 3]};
            ...
            for (int x = 0; x < outWidth; x += bw0) {
                ...
                for (int y1 = 0; y1 < bh; ++y1) {
                    ...
                    for (; x1 < bw; ++x1) {
                        ...
                        short src_x = saturate_cast_short(x_value >> INTER_BITS);
                        short src_y = saturate_cast_short(y_value >> INTER_BITS);
                        bool flag[4];
                ...
                for (int y1 = 0; y1 < bh; ++y1) {
                    ...
                    for (int x1 = 0; x1 < bw ; x1 += 4){
                        int srcX = XY_INT[4 * (y1 * bw + x1)];
                        int srcY = XY_INT[4 * (y1 * bw + x1) + 1];
                ...
                for (int y1 = 0; y1 < bh; ++y1) {
                    ...
                    for (; x1 < bw; ++x1) {
                        ...
                        xy_dec_p[x1] = ((short)((y_value & (INTER_TAB_SIZE - 1)) * INTER_TAB_SIZE + 
                            (x_value & (INTER_TAB_SIZE - 1)))) << 2;
                        x_value = saturate_cast_short(x_value >> INTER_BITS);
                        y_value = saturate_cast_short(y_value >> INTER_BITS);
                ...
                for (int y1 = 0; y1 < bh; ++y1) {
                    ...
                    if(cn == 1) {
                        for (; x1 < bw; x1+=4) {   
                            ...
                            float32x4_t p_s_t3 = vld1q_f32(short_tab + xy_dec_p[x1 + 3]);
                            float32x4_t src0 = {
                                src[xy_int_p[x1 << 2]], src[xy_int_p[(x1 << 2) + 1]], 
                                src[xy_int_p[(x1 << 2) + 2]], src[xy_int_p[(x1 << 2) + 3]]
                            };
                            float32x4_t src1 = {
                                src[xy_int_p[((x1 + 1) << 2)]], src[xy_int_p[((x1 + 1) << 2) + 1]], 
                                src[xy_int_p[((x1 + 1) << 2) + 2]], src[xy_int_p[((x1 + 1) << 2) + 3]]
                            };
                            float32x4_t src2 = {
                                src[xy_int_p[((x1 + 2) << 2)]], src[xy_int_p[((x1 + 2) << 2) + 1]], 
                                src[xy_int_p[((x1 + 2) << 2) + 2]], src[xy_int_p[((x1 + 2) << 2) + 3]]
                            };
                            float32x4_t src3 = {
                                src[xy_int_p[((x1 + 3) << 2)]], src[xy_int_p[((x1 + 3) << 2) + 1]], 
                                src[xy_int_p[((x1 + 3) << 2) + 2]], src[xy_int_p[((x1 + 3) << 2) + 3]]
                            };
...
::ppl::common::RetCode  WarpAffineNearestPoint<float, 1>(
    ...
    return warpAffine_nearest<float, 1>(outData, inData, 
...
::ppl::common::RetCode  WarpAffineNearestPoint<float, 3>(
    ...
    return warpAffine_nearest<float, 3>(outData, inData, 
        inHeight, inWidth, inWidthStride, 
        outHeight, outWidth, outWidthStride, 
        affineMatrix, border_type, borderValue);
}
...
::ppl::common::RetCode  WarpAffineNearestPoint<float, 4>(
    ...
    return warpAffine_nearest<float, 4>(outData, inData, 
        inHeight, inWidth, inWidthStride, 
        outHeight, outWidth, outWidthStride, 
        affineMatrix, border_type, borderValue);
}
